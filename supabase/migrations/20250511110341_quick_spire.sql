...
CREATE OR REPLACE FUNCTION public.check_notification_access(user_id uuid, n_recipient_type recipient_type, n_recipient_id INTEGER) 
RETURNS BOOLEAN AS $$
DECLARE
...
BEGIN
    ...
    RETURN (
        n_recipient_type = 'ALL'
        OR (
            n_recipient_type = user_role_val::text::recipient_type
            AND (n_recipient_id IS NULL OR n_recipient_id = user_id::text::integer)
        )
    );
END;
$$ LANGUAGE plpgsql SECURITY DEFINER;

-- Notifications policies with simplified access check
CREATE POLICY "Users can read their notifications"
  ON notifications FOR SELECT
  TO authenticated
  USING (
    public.check_notification_access(auth.uid(), recipient_type, recipient_id)
  );